...
export const AVAILABLE_MARKETS = (() => {
  const markets = MARKETS.filter(({ deprecated, name }) => {
    ...
    return (
      ...
      SUPPORTED_TOKEN_LIST.find((token) => ask(name) === token)
    );
  });
  markets.push({
    name: "GOFX/USDC",
...
const getLatestBid = async (
  ...
  pair: string,
  canBeDeprecated = false
) => {
  const [[latestBid]] = (
    await (
      await getMarket(connection, pair, canBeDeprecated)
    ).loadBids(connection)
  ).getL2(1);
  return latestBid;
};
...
const getMarketFromAddress = (address: PublicKey) =>
  AVAILABLE_MARKETS.find(
    ({ address: x }) => x.toString() === address.toString()
  );
...
const getMarketInfo = (pair: string, canBeDeprecated = false) => {
  ...
  const match = MARKETS.find(
    ({ deprecated, name }) =>
      name === pair && ((!canBeDeprecated && !deprecated) || canBeDeprecated)
  );
  if (!match) {
...
const getOrders = async (
  ...
  market: Market,
  owner: PublicKey
): Promise<Order[]> => await market.loadOrdersForOwner(connection, owner);
...
const subscribeToOrderBook = async (
  ...
  connection.onAccountChange(market.decoded[side], (account) =>
    callback(account, market)
  );
